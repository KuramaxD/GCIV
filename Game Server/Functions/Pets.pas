...
begin
  ...
  Self.MySQL:=MySQL;
  MySQL.SetQuery('SELECT PETID, PETIID, EXP, HEALTH, NAME, SLOT1, SLOT2, LEVEL, BIND FROM Pets WHERE UID = :ID');
    ...
    for i:=0 to Length(Pets)-1 do
      if (Pets[i].ID = PetID) and (Pets[i].Level >= 7) then begin
        if GetPetTransform(Pets[i].ItemID) > -1 then begin
          MySQL.SetQuery('SELECT NAME, ATKID, QUANTITY FROM Petlist WHERE PETID = :PETID');
